...
if __name__ == "__main__":
    ...
    output_proj = output_fid.create_dataset("/exchange/data_corrected", proj.shape,
                                    chunks=(1, proj.shape[1], proj.shape[2]), dtype='u8')
    output_flat = output_fid.create_dataset("/exchange/data_white_corrected", flat.shape,
                                chunks=(1, flat.shape[1], flat.shape[2]), dtype='u8')                                   
    del output_fid["/exchange/data_white"]
    output_iflat = output_fid.create_dataset("/exchange/data_white", flat.shape,
                                chunks=(1, flat.shape[1], flat.shape[2]), dtype='u8')                                                                   
    ...
    for ids in chunk(range(proj.shape[0]),128):
        ...
        fproj_part = (proj_part_shift/(flat_part+1e-5))*255
